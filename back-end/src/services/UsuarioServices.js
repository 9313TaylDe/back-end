import tabelaUsuarios from "../models/UsuarioTabela.js";
...
const postUser = async (req, res) => {
  ...
  try {
    ...
    const usuarioExite = await tabelaUsuarios.findOne({
      where: { email: email },
    });
    if (usuarioExite) {
...
const putUser = async (req, res) => {
  ...
  try {
    ...
    respostas.InternalServerError(
      res,
      "Ocorreu um na atulização das informações do usuario"
    );
  }
};
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (!usuario)
      return respostas.notFound(res, `Usuario com id= ${id} não foi encotrado`);

    respostas.noContent(res);
  } catch (error) {
    respostas.InternalServerError(
      res,
      "Ocorreu um errona remoção do usuario usuario"
    );
  }
};
